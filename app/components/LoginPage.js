...
function LoginPage() {
            ...
            Axios.post(appState.backendURL + "/login", {
              ...
              .then(response => {
                ...
                if (response.data == "Invalid login Credentials") {
                  dispatch({ type: "error", message: "Your email or password is not correct" })
                  dispatch({ type: "finishLoading" })
                } else {
                  dispatch({ type: "error", message: "Congratulation, you have successfully logged in" })
                  console.log(response.data)
                  dispatch({ type: "finishLoading" })
                  window.location.href = "/instructor/12345/dashboard"
                }
              })
              .catch(err => {
                if (err.response) {
                  ...
                  dispatch({ type: "error", message: "Your request cannot be processd" })
                  dispatch({ type: "finishLoading" })
                } else {
  ...
  // Runs once when the page loads and focuses the email field
  useEffect(() => {
            ...
            <div className="login-sidebar">
              ...
              >
                ...
                <button className="form-button button-style">
                  <div className="form-btn-text">{state.isLoading ? <div className="loading-signup"></div> : <p>LOG ME IN</p>}</div>
                </button>
              </form>
